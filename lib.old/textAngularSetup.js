...
.value('taOptions',  {
	toolbar: [
		...
		['html', /*'insertImage',*/ 'insertImageFromFile', /*'insertBreak',*/ 'insertLink', 'insertVideo', 'wordcount', 'charcount']
	],
	classes: {
		focussed: "focussed",
		toolbar: "btn-toolbar",
...
.run(['taRegisterTool', '$window', 'taTranslations', 'taSelection', '$modal', function(taRegisterTool, $window, taTranslations, taSelection, $modal){
	...
	taRegisterTool('insertImageFromFile', {
		iconclass: 'fa fa-picture-o',
		tooltiptext: taTranslations.insertImageFromFile.tooltip,
		action: function(deferred, restoreSelection){
			var modalInstance = $modal.open({
				templateUrl: 'updateImageModal.html',
				controller: 'updateImageModal',
				size: 'lg'
			});
			var v = this.$editor().wrapSelection;
			modalInstance.result.then(function (img) {
				restoreSelection();
					console.log(img);
					var embed = '<img class="img-responsive" src="' +img+ '" />';
					v('insertHTML', embed, true);
					deferred.resolve();
				}, function () {
	...
	taRegisterTool('insertBreak', {
		iconclass: 'fa fa-picture-o',
		tooltiptext: 'insert article break',
		action: function(){
				var embed = '</br><------------></br>';
				return this.$editor().wrapSelection('insertHTML', embed, true);
		}
